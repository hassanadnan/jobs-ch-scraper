			...
			try {
				...
				const detail = await detailsPage.evaluate(() => {
					...
					function extractDescription() {
						...
						// Include strong tags as pseudo-headings for pages that style headings without h2/h3/h4
						const headings = Array.from(root.querySelectorAll('h2, h3, h4, strong'));
						const wanted = /(Introduction|About the job|Ihre Aufgaben|Aufgaben|Ihr Profil|Profil|Unser Angebot|Angebot|Responsibilities|Your tasks|Requirements|What we offer|Über uns|Ueber uns|Deine Aufgaben|Deine Qualifikationen|Deine Benefits)/i;

						function collectUntilNextHeading(start) {
							const chunks = [];
							let node = start.nextElementSibling;
							while (node) {
								if (/^H2|H3|H4|STRONG$/.test(node.tagName)) break;
